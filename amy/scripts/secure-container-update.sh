...
# ============================================
# Schedule: Wednesday 04:30 AM (weekly), Daily 04:30 AM (retry)
# ============================================
# v1.1: Fixed Immich API endpoint, removed bc dependency
# v1.2: Added throttling and system health checks to prevent I/O saturation
...
# Prevents I/O saturation during updates
# ============================================
...
mkdir -p "/docker/backups/postgres/pre-upgrade"
  ...
  "critical": [
    {
      ...
      "functional_tests": ["atuin_db_access", "miniflux_db_access", "sss_db_access"],
      "integration_tests": ["ntfy_http", "miniflux_http"],
      "dependent_services": ["atuin", "miniflux", "spendspentspent", "postgres-backup"]
    },
    {
      "name": "ntfy",
      "pre_upgrade": [],
      "health_checks": ["container_running"],
      "functional_tests": ["ntfy_http"],
      "integration_tests": [],
      "dependent_services": []
    },
    {
    ...
    },
    {
      ...
      "pre_upgrade": [],
      "health_checks": ["container_running"],
      "functional_tests": ["spendspentspent_http"],
      "integration_tests": [],
      "dependent_services": []
    }
  ],
...
check_system_health() {
    ...
    # Note: variable named 'current' not 'load' to avoid awk builtin conflict
...
backup_postgres() {
    ...
    local backup_dir="/docker/backups/postgres/pre-upgrade"
...
run_health_check() {
    ...
    case "${check_type}" in
        ...
        "pg_connect")
            ...
            ;;
        "pg_databases")
            docker exec postgres psql -U postgres -c "\l" 2>/dev/null | grep -q "atuin"
            ;;
        
        # PostgreSQL functional tests (amy-specific)
        "atuin_db_access")
            docker exec postgres psql -U postgres -d atuin -c "SELECT 1" > /dev/null 2>&1
            ;;
        "miniflux_db_access")
            docker exec postgres psql -U postgres -d miniflux -c "SELECT 1" > /dev/null 2>&1
            ;;
        "sss_db_access")
            docker exec postgres psql -U postgres -d spendspentspent -c "SELECT 1" > /dev/null 2>&1
            ;;
        
        # Integration tests (amy-specific)
        "ntfy_http")
            local code=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:8888" 2>/dev/null)
            [[ "${code}" =~ ^(200|302|301)$ ]]
            ;;
        "miniflux_http")
            local code=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:8385" 2>/dev/null)
            [[ "${code}" =~ ^(200|302|301)$ ]]
            ;;
        "beszel_http")
            local code=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:8090" 2>/dev/null)
            [[ "${code}" =~ ^(200|302|301|401)$ ]]
            ;;
        "spendspentspent_http")
            local code=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:9021" 2>/dev/null)
            [[ "${code}" =~ ^(200|302|301)$ ]]
            ;;
...
weekly_scan() {
    ...
    if ! check_system_health; then
        ...
        if ! wait_for_system_recovery; then
            ...
            send_notification \
                "🚨 Weekly Scan Aborted" \
                "System overloaded, weekly scan aborted on amy" \
                "high" \
                "warning"
            return 1
        fi
    ...
    send_notification \
        ...
        "Checking all containers for updates on amy" \
        "low" \
        "hourglass"
...
daily_retry() {
    ...
    if ! check_system_health; then
        ...
        if ! wait_for_system_recovery; then
            ...
            send_notification \
                "🚨 Daily Retry Aborted" \
                "System overloaded, daily retry aborted on amy" \
                "high" \
                "warning"
            return 1
        fi
    ...
    send_notification \
        ...
        "Retrying blocked containers on amy" \
        "low" \
